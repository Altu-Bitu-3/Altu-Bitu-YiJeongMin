...
{
    int w, v;
}; //정수 w, v의 값을 갖는 info 구조체 선언

int knapsack1(int n, int k, vector<info> &product)
{
    vector<int> dp(k + 1, 0); //크기가 k+1이고 모든 값이 0인 정수 벡터
...
{
    vector<vector<int>> dp(n + 1, vector<int>(k + 1, 0)); //크기가 k+1이고 모든 값이 크기가 k+1이고 모든 값이 0인 정수 벡터인 벡터

    for (int i = 1; i <= n; i++) //i: 현재 넣으려는 물품
    {
        for (int j = 0; j < product[i].w; j++) //j: 최대 배낭 무게
        {
            //어차피 최대 배낭 무게가 물품 무게보다 작아서 물품을 배낭에 못넣음
