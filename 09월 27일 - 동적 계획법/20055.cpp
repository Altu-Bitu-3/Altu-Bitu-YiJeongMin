...
int minusPosition(int n, int pos) //인덱스 감소
{
    ...
    {
        return pos; //pos 값을 리턴한다
    }
    return n * 2 - 1; //n*2-1 값을 리턴한다
}

void second(int n, int start, int end)
{
    int cur = end; //현재 위치
    while (cur != start) //end - 1부터 start 까지 검사
    {
        cur = minusPosition(n, cur);
        int next = (cur + 1) % (n * 2); //다음 위치
        if (belt[cur].is_on && !belt[next].is_on && belt[next].power) //현재 위치에 로봇이 있고, 다음 위치에 로봇이 없으며 내구도 1이상인 경우 -> 로봇 옮김
        {
            belt[cur].is_on = false; //현재 위치에 로봇이 없음
            belt[next].power--; //다음 위치의 내구도 1 감소
            if (next != end) //내리는 위치 아닐 경우만
            {
                belt[next].is_on = true; //로봇 옮기기
            }
            if (belt[next].power == 0) //다음 위치의 내구도가 0이라면
            {
...
void third(int start)
{
    ...
    {
        belt[start].is_on = true; //올리는 위치에 로봇이 있음
        belt[start].power--; //올리는 위치의 내구도 1 감소
        if (belt[start].power == 0) //올리는 위치의 내구도가 0이라면
        {
            zero_power++; //내구도가 0인 벨트 칸의 개수 1 증가
        }
    }
...
int main() {
    ...
    //입력
    cin >> n >> k;
    belt.assign(n * 2, {0, false}); //벨트의 개수는 n*2개, 벨트의 초기값: 내구도가 0이고 로봇이 존재하지 않는다
    for (int i = 0; i < n * 2; i++)
    {
